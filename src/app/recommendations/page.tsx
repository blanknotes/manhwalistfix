      ...
      const enrichedRecs = await Promise.all(
        result.recommendations.map(async (rec) => {
          ...
          if (searchResults && searchResults.length > 0) {
            return { data: normalizeMAL(searchResults[0]), reason: rec.reason };
          }
          return null;
        })
      );
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-6xl">
        <section className="text-center space-y-4 mb-12">
          ...
          <p className="text-xl text-muted-foreground font-bold max-w-2xl mx-auto">
            Use the power of AI to analyze your reading taste accurately.
          </p>
        </section>
          ...
          <section className="space-y-12">
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center py-32 space-y-8 bg-secondary/10 rounded-[40px] border-4 border-dashed border-primary/20">
                ...
                <p className="text-3xl font-black animate-pulse text-center">Calculating Match...</p>
              </div>
              ...
              {recommendations.map((rec) => (
                <div 
                  key={rec.data.id} 
                  className="flex bg-white rounded-[2.5rem] shadow-sm border border-primary/5 hover:shadow-xl transition-all group overflow-hidden h-56"
                >
                  {/* BAGIAN GAMBAR: Paksa variant="grid" agar hanya gambar yang tampil */}
                  <div className="relative h-full aspect-[2/3] shrink-0 overflow-hidden">
                    <ManhwaCard manhwa={rec.data} variant="grid" />
                    {/* Overlay tipis agar transisi antar bagian lebih halus */}
                    <div className="absolute inset-y-0 right-0 w-8 bg-gradient-to-l from-white to-transparent" />
                  </div>
                  
                  {/* BAGIAN TEKS: Tempat judul dan alasan AI */}
                  <div className="flex-1 p-5 flex flex-col justify-center min-w-0 bg-white">
                    <Link href={`/manhwa/${rec.data.id}`}>
                      <h3 className="font-black text-xl line-clamp-2 leading-tight group-hover:text-primary transition-colors cursor-pointer mb-3">
                        {rec.data.title}
                      </h3>
                    </Link>
                    
                    {/* BOX ALASAN AI */}
                    <div className="bg-secondary/40 p-4 rounded-[1.5rem] italic text-xs font-semibold text-muted-foreground border-l-4 border-primary/30 leading-relaxed relative">
                      <Sparkles className="absolute -top-2 -right-2 w-4 h-4 text-primary/20" />
              ...
              <div className="text-center py-32 bg-secondary/10 rounded-[40px] border-4 border-dashed border-primary/10">
                ...
                <p className="text-muted-foreground font-bold text-lg text-center px-4">Add manhwa and select your preferred genres.</p>
              </div>
            )}
          </section>
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  {selectedTitles.map(title => (
                    <Badge key={title} className="bg-secondary text-secondary-foreground hover:bg-destructive hover:text-white transition-all px-4 py-2 rounded-xl font-bold cursor-default group text-sm border-none shadow-sm">
                      {title}
                      <button onClick={() => handleRemoveTitle(title)} className="ml-2"><X className="w-3 h-3" /></button>
                    </Badge>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-4 relative">
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    ...
                    <Input placeholder="Type a manhwa title..." value={inputValue} onChange={(e) => setInputValue(e.target.value)} className="rounded-2xl h-14 pl-10 bg-secondary/20 border-none font-bold" />
                  </div>
                  <Button onClick={() => handleAddTitle(inputValue)} className="h-14 w-14 rounded-2xl p-0 shrink-0" disabled={!inputValue}><Plus className="w-6 h-6" /></Button>
                </div>
                ...
                {(suggestions.length > 0 || isSearchingSuggestions) && (
                  <div className="absolute top-full left-0 w-full mt-2 bg-white rounded-2xl shadow-2xl border border-primary/10 z-50 overflow-hidden">
                    {isSearchingSuggestions ? (
                      <div className="p-4 flex items-center justify-center gap-2 text-muted-foreground text-sm font-bold"><Loader2 className="w-4 h-4 animate-spin" /> Searching...</div>
                    ) : (
                      <ScrollArea className="max-h-60">
                        {suggestions.map((s) => (
                          <button key={s.mal_id} onClick={() => handleAddTitle(s.title)} className="w-full p-4 text-left hover:bg-primary/5 border-b last:border-none flex items-center gap-3 transition-colors">
                ...
                <Button onClick={handleGetRecommendations} disabled={loading || selectedTitles.length < 1} className="w-full h-16 rounded-[2rem] text-xl font-black shadow-2xl shadow-primary/30 transition-all hover:scale-[1.02]">
